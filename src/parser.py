...
class Node:
	...
	def __init__(self,type,children,leaf,typelist=None,seqNo=1,order='',isLeaf=False):
		...
		for letter in order:
			if letter == 'l':
				if leaf[leafno] != None:
					term = Node(leaf[leafno], [], [],seqNo = count, isLeaf=True) 
...
def p_AndExpression(p):
	...
	if len(p) ==  4:
		p[0] = Node("AndExpression", [p[1],p[3]], [p[2]],typelist = p[1].typelist,order='clc')
	else:
		p[0] = Node("AndExpression", [p[1]],[],order='c')	
...
def p_UnaryExpression(p):
	...
	if len(p) ==  3:
		...
		p[0] = Node("MultiplicativeExpression", [p[1]],[],order='c') 
