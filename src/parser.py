...
def p_VariableDeclarator1(p):
	...
	if len(p)==4:
		...
		currentScope.InsertVar(p[1],0,p[3].typelist[0])
		p[0] = Node("VariableDeclarator1", [p[3], p[5], p[7]],[p[1],p[2], p[4], p[6]], order="llclclc",code=p[5].code+p[7].code)
	elif p[2] == ':':
		currentScope.InsertVar(p[1],0,p[3].typelist[0])
		p[0] = Node("VariableDeclarator1", [p[3], p[5]],[p[1],p[2], p[4]], order="llclc",code=p[5].code)
	else:
		if(p[3].typelist[0] == 'object'):
			currentScope.SetObjectName("temp", p[1])
			currentScope.Dumper(currentScope.LookUpObject(p[1]),symbol_file)
			...
			currentScope.InsertVar(p[1],0,p[3].typelist[0])
		p[0] = Node("VariableDeclarator1", [p[3], p[5]],[p[1],p[2], p[4]], order="llclc",code=p[3].code)
	print p[0].code
...
def p_MethodDeclaration(p):
	...
	#TODO I dont know how to do it so i am appending Method body (smsharma)
	p[0] = Node("MethodDeclaration", [p[1], p[2]],[], order="cc",code=p[2].code)
...
def p_VariableDeclarationBody(p):
	...
	#		sys.exit("Error: ", p[1]," : ", p[3].typelist[0], " = ", p[5].typelist[0], " type mismatch")
		currentScope.InsertVar(p[1],0,p[3].typelist[0])
		p[0] = Node(p[4],[p[3],p[5]],[p[1],p[2]], order="llcc",isLeaf=True,code=p[5].code)
	else:
		if(p[3].typelist[0] == 'object'):
			currentScope.SetObjectName("temp", p[1])
			currentScope.Dumper(currentScope.LookUpObject(p[1]),symbol_file)
			...
			currentScope.InsertVar(p[1],0,p[3].typelist[0], length= p[3].typelist[1])
		p[0] = Node(p[2],[p[3]],[p[1]], order="lc",isLeaf=True,code=p[3].code)
...
def p_Assignment(p):
	...
	if p[2]=="=":
		if not allowed(p[1].typelist[0], p[3].typelist[0]) :
			...
			Error = Error + 1
	#	nodename = newtemp()
	#	print nodename,"I am in Assignment"
...
<<<<<<< HEAD
			...
			code.append("call," + p[1].type)
			# I haven't appended the return value
			p[0] = Node("MethodInvocation", [p[1], p[3]], [ ],typelist = value.returnType , order='cc',code =p[3].code + code)
	# elif len(p) ==  4:
...
def p_ArgumentList(p):
	...
	#	print p[0].code
		...
		p[0] = Node('ArgumentList',[p[1],p[3]],[p[2]],typelist = p[1].typelist + p[3].typelist,order='clc',code=p[1].code+p[3].code)
